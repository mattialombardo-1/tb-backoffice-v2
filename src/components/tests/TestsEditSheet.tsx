...
import type { Test, TestSyllabusItem, UpdateTestPayload } from '@/lib/types/tests';
...
interface TestsEditSheetProps {
  test: Test | null;
  onSuccess: (updated: Test) => void;
  onCancel: () => void;
}

export function TestsEditSheet({ test, onSuccess, onCancel }: TestsEditSheetProps) {
  ...
  // ── Full test fetch (to get syllabus) ─────────────────────────────────────
  const testDetailQuery = useQuery({
    queryKey: ['tests', test?.id],
    queryFn: ({ signal }) => testsService.getById(client, test!.id, signal),
    enabled: !!test?.id,
    staleTime: 0,
  });
  ...
  // ── Sync form when test detail loads ─────────────────────────────────────
  useEffect(() => {
    const detail = testDetailQuery.data ?? test;
    if (!detail) return;
    setName(detail.name);
    setYear(detail.year != null ? String(detail.year) : '');
    setSelectedBrandIds(detail.brands.map((b) => b.id));
    setSyllabus(detail.syllabus ?? []);
    setScores({
      correct: String(detail.defaultScores.correct),
      empty: String(detail.defaultScores.empty),
      wrong: String(detail.defaultScores.wrong),
    });
    setIsSubmitting(false);
    setDraft(EMPTY_DRAFT);
  }, [testDetailQuery.data, test]);
  ...
  const isLoadingDetail = testDetailQuery.isLoading && !!test;
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent side="right" className="w-full max-w-2xl flex flex-col p-0">
          ...
          {isLoadingDetail ? (
            ...
            <>
              ...
              <div className="grid grid-cols-[1fr_auto] gap-3 items-end">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="edit-test-year">Anno</Label>
                  <Input
                    id="edit-test-year"
                    type="number"
                    value={year}
        ...
        <div className="border-t px-6 py-4 flex justify-end gap-2 shrink-0">
          ...
          <Button onClick={handleSubmit} disabled={!canSubmit || isLoadingDetail}>
